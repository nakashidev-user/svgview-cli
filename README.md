...
- サイドパネルから保存済みSVGを論理削除できます。削除済みアイテムは一覧に表示されません。
- サイドパネルの「ファイルを開く」ボタンから、OSのファイル選択ダイアログでSVGを追加できます（内容ハッシュで重複検知）。
  - ブラウザのセキュリティ制約により、この経路では元ファイルパスを保持できません。編集→自動リロードを使いたい場合は `svgview <path>` でCLIから起動してください。
- 外部通信は行いません。
- ランタイム依存パッケージはありません。
...
- `POST /library`: SVGの本文（`image/svg+xml`）をボディに乗せると、ライブラリに追加します。ファイル名は `X-Svgview-Filename` ヘッダ（URLエンコード）で渡します。リクエストサイズ上限は10MB。内容のSHA-256で重複検知し、既存と一致した場合は新規作成せず既存IDを返します。
- `GET /library/:id`: 保存済みSVGを返します。
- `DELETE /library/:id`: 保存済みSVGを論理削除します。
